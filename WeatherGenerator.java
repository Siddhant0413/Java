...
public class WeatherGenerator {
    ...
    {
        int [] array = new int[numberOfDaysInMonth[month - 2]];
        double fDay = StdRandom.uniform();
        double dryWetPer = 0.0;
        double wetwetPer = 0.0;
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) 
    {
        ...
        for(int i = 0; i < forecast.length; i++)
        {
            if(forecast[i] == mode && curr == 0)
            {
                curr++;
                prev = forecast[i];
            }
            else if(forecast[i] == mode && forecast[i - 1] == forecast[i])
            {
                curr++;
                prev = forecast[i];
            }
